...
稳态速度误差、力矩饱和比例以及平顺性和控制量代理指标。因此，本文的“鲁棒性”含义限定为
固定扰动场景下的初步对比和有效性边界讨论，而非多随机种子 Monte Carlo 意义上的统计鲁棒性证明。
...
$1.5\times10^6$ 步训练协议下的平均姿态 RMSE 为 \SI{0.233}{\degree}，平均速度 RMSE 为
...
需要强调的是，本文研究对象是题目参数驱动的二维降阶轮腿模型，而不是含真实髋膝关节角、轮速、连杆质量、
转动惯量和接触切换的完整多刚体机器人模型。报告中的控制器优劣结论只针对该降阶模型、参数、限幅和场景成立。
...
生成连续高度、坡度和曲率。
该状态没有显式关节角、轮速、连杆姿态和接触模式，因此更接近带轮式水平驱动的竖直伸缩腿模型；
大腿和小腿长度主要用于腿长可达性和观测量构造。
...
\end{align}
计算。这里的切向力限幅对应二维库仑摩擦约束，$\max(F_n,1)$ 用于法向力很小时的数值保护；它不是物理离地接触模型，
只避免数值积分在 $F_n\approx0$ 时出现零摩擦边界奇异。正式评估轨迹中腿长和法向力均未进入长期离地状态，因此该保护
主要影响极端瞬态附近的数值稳定性。
...
\end{equation}
上式采用小坡度等效处理：$F_n$ 和 $F_t$ 保留接触力解释，但在动力学中分别作为竖直支撑力和水平驱动力使用；
坡度影响主要通过 $mgh'(x)$ 和腿长变化率进入。对于高坡度、大曲率或离地碰撞工况，该近似需要由地形坐标系力分解
和多刚体接触模型替代。
...
因此，表格中的“未终止”仅表示未触发上述粗粒度失败条件，并不等同于真实系统安全。当前判据尚未包含速度发散、
...
\end{align}
由于速度指令由零平滑上升到目标值，全时域速度 RMSE 会混入启动过程。作为稳态评价，本文还统计
$t\ge\SI{2}{s}$ 后的速度误差：
\begin{equation}
...
该设计把膝关节视为主要伸腿支撑执行器，给直接 PPO 引入了不同于 LQR、WBC/QP 和残差 PPO 的动作先验；
因此本文把直接 PPO 结果解释为当前动作空间设计下的经验表现，而不把它视为与其它控制律完全等价的公平算法比较。
...
其中 C2 只使用固定随机种子 7，不能代表随机地形统计分布；它应理解为一个平滑随机地形样例。
...
外力在无扰动、脉冲和恒定扰动中随机，幅值大体限制在题目范围内。本文只采用单随机种子模型，
未用多模型统计均值和标准差；因此固定测试结果不能声称为严格泛化性能证明。
...
对平台高度和躯干姿态赋予了较高权重，并且与前向仿真共享降阶模型和地形信息。在同一受控训练协议下，
...
场景 E 的峰值外力为 \SI{160}{N}，超出题目给定的 \SI{100}{N} 上限，因此表~\ref{tab:req-vs-stress}
把题目范围内七场景和包含 E 的八主场景分开列出。两种平均方式不改变主要排序，但报告后文把 E 仅作为超规格压力测试解释。
...
表~\ref{tab:sat-channel} 显示，整体饱和比例主要由膝关节通道贡献，髋关节通道在该数据集中几乎不触及 $0.98$ 倍限幅。
因此单一平均饱和比例只能作为粗指标；定位执行器瓶颈时应使用分通道统计。

\subsection{PPO 受控训练协议比较}
表~\ref{tab:ppo-controlled} 单独列出三种 PPO 在主比较中的结果。三者使用相同的训练分布、训练步数、并行环境数、
随机种子、学习率、熵系数和归一化动作惩罚；因此该表主要比较动作映射和基础控制器结构的影响。

\begin{table}[htbp]
\centering
\caption{三种 PPO 的受控训练协议结果}
...
学到了保守的低能量策略，而不是有效的高速跟踪策略。由于三种动作到物理力矩的尺度仍不同，表中结论应理解为
当前动作映射设计的受控实验结果，而不是对所有直接力矩 PPO 形式的否定。
...
\caption{G 边界组合}
\end{subfigure}
\caption{受控 PPO 协议下的代表性对照曲线}
\label{fig:ppo-controlled}
\end{figure}

图~\ref{fig:ppo-controlled} 对应表~\ref{tab:ppo-controlled} 的受控协议数据。
第~\ref{sec:scene-analysis} 节的五控制器分场景图同样来自表~\ref{tab:aggregate} 的主比较数据。
...
LQR、前馈残差 PPO 和直接 PPO 均出现持续速度偏差；WBC/QP 的速度曲线更接近指令速度。图~\ref{fig:scene-b}
...
单独列出 WBC/QP 与直接 PPO：前者的高度和姿态曲线接近零线，后者在该受控协议下速度偏差明显。
...
表~\ref{tab:static-push} 表明，\SI{0}{m/s} 脉冲场景下五种控制器均可维持粗安全条件；
...
理由是其任务优先级与约束可显式解释，并在边界场景 G 中给出最小高度、姿态误差和最低速度误差。PD 残差 PPO
可作为学习策略对照；在当前证据下，直接 PPO 尚未作为速度补偿或安全关键主控制器使用。LQR 保留为基线和故障回退方案。
...
\begin{enumerate}
  ...
  \item \SI{0}{m/s} 静止抗扰检查支持主实验趋势，但本文实验仍是确定性课程仿真比较，不是实机稳定性证明。下一步应加入多随机种子、参数失配、传感器噪声、时延、接触切换和高保真多刚体模型。
\end{enumerate}
